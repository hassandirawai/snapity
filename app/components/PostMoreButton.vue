  ...
  <DropdownMenu>
    <DropdownMenuTrigger>
      <Button size="icon" variant="ghost" :class="props.class">
        <Icon icon="fluent:more-horizontal-20-regular" class="text-muted-foreground" size="24" />
      </Button>
    </DropdownMenuTrigger>
    <DropdownMenuContent>
      <DialogTrigger as-child>
        <DropdownMenuItem v-if="loggedInUser?.id === post.authorId">
          <span class="flex gap-3 items-center text-destructive">
            <Icon icon="fluent:delete-20-regular" size="18" />
            Delete
          </span>
        </DropdownMenuItem>
      </DialogTrigger>
      <DropdownMenuItem @click="copy(postLink)">
        <Icon icon="fluent:copy-select-20-regular" size="18" />
        Copy Link
      </DropdownMenuItem>
    </DropdownMenuContent>
  </DropdownMenu>
